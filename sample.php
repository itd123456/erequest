<?php
	...
	else
	{
		...
		if ($area == 'Makati')
		{
			...
			ON r.user = u.user
			WHERE app_date BETWEEN '$start' AND '$end'
			AND u.branch IN ('Cavite', 'Muntinlupa', 'Las Piñas', 'Intramuros', 'Calamba', 'Sucat', 'Manila', 'Harrison Plaza', 'Imus', 'Makati')";
		}
		else if ($area == 'GMA')
		{
			$sql = "SELECT r.item, u.Department, u.branch
			...
			ON r.user = u.user
			WHERE app_date BETWEEN '$start' AND '$end'
			AND u.branch IN ('POEA', 'Lagro', 'Valenzuela', 'Quezon Avenue', 'Tandang Sora', 'Antipolo', 'Tanay', 'Marikina', 'Cainta', 'SME Marikina', 'SME San Mateo', 'SME Antipolo')";
		}
		else if ($area == 'Sure Cycle')
		{
			$sql = "SELECT r.item, u.Department, u.branch
					...
					ON r.user = u.user
					WHERE app_date BETWEEN '$start' AND '$end'
					AND u.branch IN ('Digos Trike', 'SC Koronadal', 'SC Panabo')";
		}
		else if ($area == 'North Branches')
		{
			$sql = "SELECT r.item, u.Department, u.branch
			FROM requisition r
			JOIN user_data u
			ON r.user = u.user
			WHERE app_date BETWEEN '$start' AND '$end'
			AND u.branch IN ('Baguio', 'Olongapo', 'Dagupan', 'Bataan', 'Meycauayan', 'Baliuag', 'San Jose', 'La Trinidad', 'La Union', 'Angeles', 'Dau', 'Malolos', 'Santiago', 'San Fernando, PA', 'Tarlac', 'Laoag', 'Cabanatuan', 'Tuguegarao', 'Cauayan', 'Gapan', 'Baler', 'Lipa', 'San Pablo', 'Naga')";
		}
		else if ($area == 'VisMin')
		{
			$sql = "SELECT r.item, u.Department, u.branch
			...
			ON r.user = u.user
			WHERE app_date BETWEEN '$start' AND '$end'
			AND u.branch IN ('Cebu', 'Consolacion', 'Mandue', 'Bacolod', 'Iloilo', 'Roxas', 'Dumaguete', 'Kabankalan', 'Tacloban', 'Tagbilaran', 'Cagayan De Oro', 'Malaybalay', 'Butuan', 'Valencia', 'General Santos', 'Davao', 'Digos', 'Tagum', 'Koronadal', 'Kidapawan')";
		}
	}
